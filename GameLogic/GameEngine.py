from .constants import ROWS, COLS, BOARD
import time, math
...
class GameState:
    ...
    ############ make ant move on the BOARD and on the GUI takes screen obj MOVE obj and ant obj##################
    ...
    def get_next_move(self, node_parent):
        ...
        self.old = path[-1]
        return path[-1]
    # $$$ calculate the shortest path between the spider and the ant and returns the next step for the spider $$$$$
    ...
    # *Note: DFS is not working as t should be  
        ...
        while stack:
            ...
            for i in range(len(self.validRowMoves)):
                ...
                if (neighbor_x, neighbor_y) not in visited and is_valid(neighbor_x, neighbor_y):
                    ...
                    node_parent[(neighbor_x, neighbor_y)] = current
                if (neighbor_x, neighbor_y) == (self.ant.get_pos()):
                    return node_parent
        return False
